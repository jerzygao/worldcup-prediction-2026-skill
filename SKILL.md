...
7. **完整逻辑回归重拟合（重新校准 12 个模型参数）：**

```bash
# Step 1: 重建特征（确保 results.csv 包含最新赛果）
...
⚠️ **不要手动调 drawBias/drawEloPenalty/homeCoef 等参数来追赛果。** 2026-06-15 的教训：v2 hotfix 把 drawBias 从 -0.15 调到 -0.03 后，验证集平局预测飙到 31.9%（实际 23.7%），属于典型的在小样本(10场)上 overfit。正确的做法是跑完整重拟合，让优化器在 4400+ 验证场次上找平衡。详见 `references/draw-underprediction-diagnosis.md`。
