...
function parseArgs(argv) {
  const args = {
    ...
    output: "output/fit-elo-form.json",
    iterations: "50000",
    since: "1990-01-01",
...
function predict(row, p) {
  ...
  const strength = p.eloCoef * elo + p.formCoef * form + p.homeCoef * homeAdv;
  const drawLogit =
    p.drawBias -
...
function defaultRanges() {
  return {
    ...
    homeCoef: [0.0, 0.75],
    drawBias: [-0.9, 0.7],
    drawEloPenalty: [0.0, 0.7],
    drawFormPenalty: [0.0, 1.2],
    drawNeutralBoost: [-0.2, 0.3]
  };
}

function loadRanges(filePath) {
  if (!filePath) return defaultRanges();
  const raw = JSON.parse(fs.readFileSync(filePath, "utf8"));
  return raw.suggestedRanges ?? raw;
}

function narrowRanges(best) {
  return {
    eloCoef: [Math.max(0.01, best.eloCoef * 0.75), best.eloCoef * 1.25],
    formCoef: [Math.max(0, best.formCoef - 0.25), best.formCoef + 0.25],
    homeCoef: [Math.max(0, best.homeCoef - 0.15), best.homeCoef + 0.15],
    drawBias: [best.drawBias - 0.25, best.drawBias + 0.25],
    drawEloPenalty: [Math.max(0, best.drawEloPenalty - 0.15), best.drawEloPenalty + 0.15],
    drawFormPenalty: [Math.max(0, best.drawFormPenalty - 0.25), best.drawFormPenalty + 0.25],
    drawNeutralBoost: [best.drawNeutralBoost - 0.12, best.drawNeutralBoost + 0.12]
  };
}
...
function main() {
  ...
  const rows = readCsv(args.input)
    ...
    .filter((row) => row.date >= args.since && ["H", "D", "A"].includes(row.actual) && Number.isFinite(row.eloDiff) && Number.isFinite(row.recentFormDiff));
  ...
  const rounds = [
    { name: "wide", iterations: Math.floor(Number(args.iterations) * 0.55), ranges: loadRanges(args.rangeFile) }
  ];

  let best = null;
  const top = [];
  for (const round of rounds) {
    for (let i = 0; i < round.iterations; i += 1) {
      const params = randomParams(rand, round.ranges);
      const metrics = scoreRows(fitTrain, params);
      const candidate = { params, train: metrics };
      if (!best || metrics.logLoss < best.train.logLoss) best = candidate;
      top.push(candidate);
    }
  }
  ...
  top.sort((a, b) => a.train.logLoss - b.train.logLoss);
  const result = {
    ...
    rows: {
      total: rows.length,
      train: train.length,
      fitTrain: fitTrain.length,
      validation: validation.length
    },
    best: {
      params: Object.fromEntries(Object.entries(best.params).map(([k, v]) => [k, Number(v.toFixed(6))])),
      fitTrain: Object.fromEntries(Object.entries(best.train).map(([k, v]) => [k, typeof v === "number" ? Number(v.toFixed(6)) : v])),
    ...
    top10: top.slice(0, 10).map((item) => ({
      ...
      trainLogLoss: Number(item.train.logLoss.toFixed(6)),
      trainBrier: Number(item.train.brierScore.toFixed(6)),
      trainAccuracy: Number(item.train.accuracy.toFixed(6))
    }))
  };
