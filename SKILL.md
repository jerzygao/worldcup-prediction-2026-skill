...
| `references/cn-report-translation.md` | 报告中英文残留全量中文化 |
...
每轮比赛结束后，按以下步骤更新外部数据并生成验证报告：

```bash
# 1. 拉取最新赛果（Sailing MCP）
mcporter call sailing-sports-mcp tteagt --args '{"query": "今天世界杯比赛结果", "project": "FBL"}'

# 2. 运行赛后更新脚本（更新 Elo、team_stats、预测日志、验证报告）
python3 scripts/post-match-update.py
```

该脚本执行以下操作：
- 读取 Sailing 返回的赛果，匹配预测数据
- 更新 `data/external/poisson-elo.json`（K=32 Elo 公式）
- 更新 `data/external/poisson-team-stats.json`（加权平均：0.7 旧 + 0.3 新）
- 写入 `output/prediction-log.json`（逐场预测 vs 实际对比）
- 生成 `output/post-match-verification.md`（方向/比分准确率统计）
